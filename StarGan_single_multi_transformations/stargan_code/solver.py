    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                filenames = [
                    dataset.train_dataset[idx][0] if dataset.mode == 'train' else dataset.test_dataset[idx][0]
                    for idx in range(i * data_loader.batch_size, min((i + 1) * data_loader.batch_size, len(dataset)))
                ]
                ...
                for j, c_trg in enumerate(c_trg_list):
                    ...
                    for k, (img, filename) in enumerate(zip(x_fake, filenames)):  # Loop through batch
                        base_filename = os.path.splitext(filename)[0]  # Remove extension
                        attribute = self.selected_attrs[j]  # Get the attribute name
                        result_filename = f"{base_filename}_{attribute}.jpg"
